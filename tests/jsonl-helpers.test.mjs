...
import { appendHookLogEntryIfConfigured } from "../extensions/pi-evo-research/hooks.ts";
import {
  extractEvoResearchSessionName,
  hasEvoResearchConfigHeader,
  isEvoResearchConfigEntry,
  isEvoResearchRunEntry,
  parseJsonlEntry,
  reconstructJsonlState,
} from "../extensions/pi-evo-research/jsonl.ts";
...
test("hook entries are skipped when identifying run entries", () => {
  ...
  assert.equal(isEvoResearchConfigEntry(hookEntry), false);
  assert.equal(isEvoResearchRunEntry(hookEntry), false);
  assert.equal(isEvoResearchRunEntry(runEntry), true);
});

test("hook-only jsonl does not count as having a config header", () => {
  const jsonl = '{"type":"hook","stage":"before","exit_code":0}\n';

  assert.equal(hasEvoResearchConfigHeader(jsonl), false);
});
...
test("session name comes from the first config entry, not the first line", () => {
  ...
  assert.equal(hasEvoResearchConfigHeader(jsonl), true);
  assert.equal(extractEvoResearchSessionName(jsonl), "Hook-safe session");
});
...
test("hook observability does not create jsonl before config exists", () => {
  const tempDir = fs.mkdtempSync(path.join(tmpdir(), "pi-evo-research-hooks-"));
  const jsonlPath = path.join(tempDir, "evo-research.jsonl");
  const result = {
    fired: true,
    stdout: "",
    stderr: "",
