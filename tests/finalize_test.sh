...
# Creates temp git repos, simulates evo-research sessions, and verifies behavior.

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
FINALIZE="$SCRIPT_DIR/../skills/pi-evo-research-finalize/finalize.sh"
...
# Create a fresh test repo with a simulated evo-research session
...
setup_repo() {
  ...
  # Evo Research branch
  git checkout -b evo-research/test-session --quiet

  # Session files (should not end up in branches)
  echo '{"type":"config","metricName":"ms","metricUnit":"ms","bestDirection":"lower"}' > evo-research.jsonl
  echo "# Evo Research session" > evo-research.md
  echo "#!/bin/bash" > evo-research.sh
  echo "- try X" > evo-research.ideas.md
...
EOF
  ...
  git rev-parse "evo-research/test/01-optimize-a" >/dev/null 2>&1 || { fail_test "basic two groups" "Branch 01 not created"; cleanup_repo "$REPO"; return; }
  git rev-parse "evo-research/test/02-optimize-b" >/dev/null 2>&1 || { fail_test "basic two groups" "Branch 02 not created"; cleanup_repo "$REPO"; return; }

  # Check each branch starts from BASE
  local BASE_OF_A BASE_OF_B
  BASE_OF_A=$(git merge-base "evo-research/test/01-optimize-a" main)
  BASE_OF_B=$(git merge-base "evo-research/test/02-optimize-b" main)
  ...
  COUNT_A=$(git rev-list --count "$BASE"..evo-research/test/01-optimize-a)
  COUNT_B=$(git rev-list --count "$BASE"..evo-research/test/02-optimize-b)
  ...
  A_CONTENT=$(git show evo-research/test/01-optimize-a:file_a.txt)
  B_CONTENT=$(git show evo-research/test/02-optimize-b:file_b.txt)
  ...
  A_FILEB=$(git show evo-research/test/01-optimize-a:file_b.txt)
  B_FILEA=$(git show evo-research/test/02-optimize-b:file_a.txt)
...
EOF
  ...
  for f in evo-research.jsonl evo-research.sh evo-research.md evo-research.ideas.md; do
    if git show "evo-research/test/01-all":"$f" &>/dev/null 2>&1; then
...
EOF
  ...
  [ "$CURRENT" = "evo-research/test-session" ] || { fail_test "rollback on failure" "Not on original branch: $CURRENT"; cleanup_repo "$REPO"; return; }

  # No leftover branches
  if git rev-parse "evo-research/test/01-good" >/dev/null 2>&1; then
...
EOF
  ...
  echo "$OUTPUT" | grep -q "evo-research.ideas.md" || { fail_test "summary output" "Missing ideas in output"; rm -f "$GROUPS_JSON"; cleanup_repo "$REPO"; return; }

  # No summary file should be written to disk
  [ ! -f "pi-evo-research-finalize-summary.md" ] || { fail_test "summary output" "Summary file written to disk — should only print"; rm -f "$GROUPS_JSON"; cleanup_repo "$REPO"; return; }
...
test_no_args() {
  TESTS_RUN=$((TESTS_RUN + 1))
  local scratch_dir="${TMPDIR:-$HOME/tmp}"
  mkdir -p "$scratch_dir"
  cd "$scratch_dir"
...
test_branch_collision() {
  ...
  git branch "evo-research/test/01-optimize-a" "$BASE"
...
test_missing_groups_json() {
  ...
  cd "$scratch_dir"
  local OUTPUT
  if OUTPUT=$(bash "$FINALIZE" "$scratch_dir/nonexistent-groups.json" 2>&1); then
...
EOF
  ...
  git rev-parse "evo-research/test/01-all" >/dev/null 2>&1 || { fail_test "single group" "Branch not created"; cleanup_repo "$REPO"; return; }

  # Should have both file changes
  local A B
  A=$(git show evo-research/test/01-all:file_a.txt)
  B=$(git show evo-research/test/01-all:file_b.txt)
...
test_nested_session_artifacts() {
  ...
  git checkout -b evo-research/nested-test --quiet

  # Session files in a subdirectory (like world's libraries/javascript/polaris/)
  echo '{"type":"config"}' > libs/polaris/evo-research.jsonl
  echo "# session" > libs/polaris/evo-research.md
  echo "#!/bin/bash" > libs/polaris/evo-research.sh
  echo "- idea" > libs/polaris/evo-research.ideas.md
  echo "#!/bin/bash" > libs/polaris/evo-research.checks.sh
...
EOF
  ...
  # Branch should only have component.ts, not any evo-research.* files
  local BRANCH="evo-research/test/01-optimize"
...
EOF
  ...
  git rev-parse "evo-research/test/01-optimize-a" >/dev/null 2>&1 || { fail_test "verify failure leaves branches" "Branch was deleted"; cleanup_repo "$REPO"; return; }

  # Should be back on original branch
  local CURRENT
  CURRENT=$(git branch --show-current 2>/dev/null || echo "")
  [ "$CURRENT" = "evo-research/test-session" ] || { fail_test "verify failure leaves branches" "Not on original branch: $CURRENT"; cleanup_repo "$REPO"; return; }
...
EOF
  ...
  local CURRENT
  CURRENT=$(git branch --show-current 2>/dev/null || echo "")
  [ "$CURRENT" = "evo-research/test-session" ] || { fail_test "stash pop on rollback" "Not on original branch: $CURRENT"; cleanup_repo "$REPO"; return; }
...
test_three_groups() {
  ...
  git checkout -b evo-research/three-test --quiet
...
EOF
  ...
  git rev-parse "evo-research/three/01-a" >/dev/null 2>&1 || { fail_test "three groups" "Branch 01 missing"; cleanup_repo "$REPO"; return; }
  git rev-parse "evo-research/three/02-b" >/dev/null 2>&1 || { fail_test "three groups" "Branch 02 missing"; cleanup_repo "$REPO"; return; }
  git rev-parse "evo-research/three/03-c" >/dev/null 2>&1 || { fail_test "three groups" "Branch 03 missing"; cleanup_repo "$REPO"; return; }

  # Each branch is independent — 1 commit from base
  for b in evo-research/three/01-a evo-research/three/02-b evo-research/three/03-c; do
    local C
  ...
  [ "$(git show evo-research/three/01-a:file_a.txt)" = "optimized_a" ] || { fail_test "three groups" "01-a: wrong file_a"; cleanup_repo "$REPO"; return; }
  [ "$(git show evo-research/three/01-a:file_b.txt)" = "original_b" ] || { fail_test "three groups" "01-a: has file_b changes"; cleanup_repo "$REPO"; return; }
  [ "$(git show evo-research/three/02-b:file_b.txt)" = "optimized_b" ] || { fail_test "three groups" "02-b: wrong file_b"; cleanup_repo "$REPO"; return; }
  [ "$(git show evo-research/three/02-b:file_a.txt)" = "original_a" ] || { fail_test "three groups" "02-b: has file_a changes"; cleanup_repo "$REPO"; return; }
  [ "$(git show evo-research/three/03-c:file_c.txt)" = "optimized_c" ] || { fail_test "three groups" "03-c: wrong file_c"; cleanup_repo "$REPO"; return; }
