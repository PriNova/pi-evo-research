...
} from "../extensions/pi-evo-research/compaction.ts";

function withTempWorkDir(fn) {
  const dir = fs.mkdtempSync(path.join(tmpdir(), "pi-evo-research-compact-"));
  try {
    fn(dir);
  } finally {
...
function writeJsonlLines(workDir, lines) {
  fs.writeFileSync(path.join(workDir, "evo-research.jsonl"), lines.join("\n") + "\n");
}

test("summary contains all persisted sources when present", () => {
  withTempWorkDir((workDir) => {
    fs.writeFileSync(path.join(workDir, "evo-research.md"), "# Rules\nDo not cheat.");
    fs.writeFileSync(path.join(workDir, "evo-research.ideas.md"), "- Try memoization\n- Try parallelism");
    ...
    const summary = buildEvoResearchCompactionSummary(evoResearchSummaryPathsFor(workDir));

    assert.match(summary, /# EvoResearch Compaction Summary/);
    ...
    assert.match(summary, /## Experiment Rules \(evo-research\.md\)/);
    assert.match(summary, /Do not cheat\./);
    assert.match(summary, /## Ideas Backlog \(evo-research\.ideas\.md\)/);
    ...
    assert.match(summary, /If you need more details, read additional lines from evo-research\.jsonl\./);
  });
});
