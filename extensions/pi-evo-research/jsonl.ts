...
export interface EvoResearchConfigEntry extends JsonlEntry {
  type: "config";
  name?: string;
  metricName?: string;
  metricUnit?: string;
...
export interface EvoResearchRunEntry extends JsonlEntry {
  run: number;
}
...
function updateConfig(state: ReconstructedJsonlState, entry: EvoResearchConfigEntry): void {
...
function runFrom(entry: EvoResearchRunEntry, segment: number): ReconstructedRun {
  return {
...
export function isEvoResearchConfigEntry(entry: unknown): entry is EvoResearchConfigEntry {
  return isObjectRecord(entry) && entry.type === "config";
}

export function isEvoResearchRunEntry(entry: unknown): entry is EvoResearchRunEntry {
  return isObjectRecord(entry) && typeof entry.run === "number";
}

function firstConfigEntry(jsonlContent: string): EvoResearchConfigEntry | null {
  for (const line of nonEmptyLines(jsonlContent)) {
    const entry = parseJsonlEntry(line);
    if (isEvoResearchConfigEntry(entry)) return entry;
  }
  return null;
}

export function hasEvoResearchConfigHeader(jsonlContent: string): boolean {
  return firstConfigEntry(jsonlContent) !== null;
}

export function extractEvoResearchSessionName(jsonlContent: string): string {
  return firstConfigEntry(jsonlContent)?.name || "EvoResearch";
}
...
export function reconstructJsonlState(jsonlContent: string): ReconstructedJsonlState {
  ...
  for (const line of nonEmptyLines(jsonlContent)) {
    ...
    if (isEvoResearchConfigEntry(entry)) {
    ...
    if (!isEvoResearchRunEntry(entry)) continue;
