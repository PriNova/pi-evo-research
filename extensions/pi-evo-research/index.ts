 ...
 * - Adds evo-research guidance to the system prompt and points the agent at evo-research.md
 * - Injects evo-research.md into context on every turn via before_agent_start
 */
...
import {
  parseJsonlEntry,
  isEvoResearchRunEntry,
  extractEvoResearchSessionName,
  reconstructJsonlState,
} from "./jsonl.ts";
import {
  evoResearchSummaryPathsFor,
...
interface EvoResearchRuntime {
  evoResearchMode: boolean;
...
const RunParams = Type.Object({
  ...
  checks_timeout_seconds: Type.Optional(
    Type.Number({
      description:
        "Kill evo-research.checks.sh after this many seconds (default: 300). Only relevant when the checks file exists.",
    })
  ),
});
 ...
 * then check that the core command is evo-research.sh invoked via a known
 * pattern. Rejects chaining tricks like "evil.py; evo-research.sh" because
 * we require evo-research.sh to be the *first* real command.
 */
function isEvoResearchShCommand(command: string): boolean {
  ...
  // Now the core command must be evo-research.sh via a known invocation:
  //   evo-research.sh
  //   ./evo-research.sh
  //   /path/to/evo-research.sh
  //   bash [-flags] evo-research.sh
  //   bash [-flags] ./evo-research.sh
  //   bash [-flags] /path/to/evo-research.sh
  return /^(?:(?:bash|sh|source)\s+(?:-\w+\s+)*)?(?:\.\/|\/[\w/.-]*\/)?evo-research\.sh(?:\s|$)/.test(cmd);
}
...
interface EvoResearchConfig {
  maxIterations?: number;
  workingDir?: string;
}

/** Read evo-research.config.json from the given directory (always ctx.cwd) */
function readConfig(cwd: string): EvoResearchConfig {
  try {
    const configPath = evoResearchConfigPath(cwd);
...
/** Read maxExperiments from evo-research.config.json (if it exists) */
 ...
 * Reads workingDir from evo-research.config.json in ctxCwd.
...
function validateWorkDir(ctxCwd: string): string | null {
  ...
  try {
    ...
    if (!stat.isDirectory()) {
      return `workingDir "${workDir}" (from evo-research.config.json) is not a directory.`;
    }
  } catch {
    return `workingDir "${workDir}" (from evo-research.config.json) does not exist.`;
  }
  return null;
}
...
// -----------------------------------------------------------------------
// Session file paths (single source of truth for evo-research.* filenames)
// -----------------------------------------------------------------------

const evoResearchJsonlPath  = (dir: string) => path.join(dir, "evo-research.jsonl");
const evoResearchMdPath     = (dir: string) => path.join(dir, "evo-research.md");
const evoResearchIdeasPath  = (dir: string) => path.join(dir, "evo-research.ideas.md");
...
function createSessionRuntime(): EvoResearchRuntime {
  return {
    evoResearchMode: false,
...
function createRuntimeStore() {
  const runtimes = new Map<string, EvoResearchRuntime>();

  return {
    ensure(sessionKey: string): EvoResearchRuntime {
...
export default function evoResearchExtension(pi: ExtensionAPI) {
  ...
  const shortcuts = resolveEvoResearchShortcuts();
  ...
  const getRuntime = (ctx: ExtensionContext): EvoResearchRuntime =>
    runtimeStore.ensure(getSessionKey(ctx));

  const isAgentSettled = (ctx: ExtensionContext): boolean =>
    ctx.isIdle() && !ctx.hasPendingMessages();

  const hasPendingResume = (runtime: EvoResearchRuntime): boolean =>
    runtime.pendingResumeMessage !== null;

  const pausePendingResume = (runtime: EvoResearchRuntime): void => {
    if (!runtime.pendingResumeTimer) return;
    clearTimeout(runtime.pendingResumeTimer);
    runtime.pendingResumeTimer = null;
  };

  const cancelPendingResume = (runtime: EvoResearchRuntime): void => {
    pausePendingResume(runtime);
    runtime.pendingResumeMessage = null;
  };

  const markAutoResumeSent = (runtime: EvoResearchRuntime): void => {
    runtime.autoResumeTurns++;
  };

  const sendPendingResumeIfReady = (ctx: ExtensionContext, runtime: EvoResearchRuntime): void => {
    const message = runtime.pendingResumeMessage;

    if (!message) return;
    if (!runtime.evoResearchMode) {
  ...
  const schedulePendingResume = (ctx: ExtensionContext, runtime: EvoResearchRuntime, message: string): void => {
    pausePendingResume(runtime);
  ...
  const reschedulePendingResume = (ctx: ExtensionContext, runtime: EvoResearchRuntime): void => {
    if (!hasPendingResume(runtime)) return;
    schedulePendingResume(ctx, runtime, runtime.pendingResumeMessage!);
  };

  const hasRunExperimentsThisSession = (runtime: EvoResearchRuntime): boolean =>
  ...
  const shouldAutoResumeAfterTurn = (runtime: EvoResearchRuntime): boolean =>
    runtime.evoResearchMode && hasRunExperimentsThisSession(runtime);

  const shouldAutoResumeAfterCompact = (runtime: EvoResearchRuntime): boolean =>
    runtime.evoResearchMode;

  const hasReachedAutoResumeLimit = (runtime: EvoResearchRuntime): boolean =>
    runtime.autoResumeTurns >= MAX_AUTORESUME_TURNS;

  const notifyAutoResumeLimitReached = (ctx: ExtensionContext): void => {
    ctx.ui.notify(
      `EvoResearch auto-resume limit reached (${MAX_AUTORESUME_TURNS} turns)`,
      "info",
    );
  };
  ...
  const composeResumeMessage = (_ctx: ExtensionContext): string => {
    return [
      ...
      "Use the persisted evo-research state as needed, pick the most promising hypothesis, then call run_experiment + log_experiment.",
      BENCHMARK_GUARDRAIL,
    ].join(" ");
  };
  ...
  const composeCompactionResumeMessage = (_ctx: ExtensionContext): string => {
    ...
    return [
      ...
      "Do not re-read evo-research.md or evo-research.jsonl — the compaction summary already contains them.",
      BENCHMARK_GUARDRAIL,
    ].join(" ");
  };

  const evoResearchCompactionFor = (
    ctx: ExtensionContext,
    event: SessionBeforeCompactEvent,
  ) => {
    if (!getRuntime(ctx).evoResearchMode) return undefined;
    return {
      compaction: {
        summary: buildEvoResearchCompactionSummary(
          evoResearchSummaryPathsFor(resolveWorkDir(ctx.cwd)),
        ),
  ...
  const hasEvoResearchRules = (ctx: ExtensionContext): boolean =>
    fs.existsSync(evoResearchMdPath(resolveWorkDir(ctx.cwd)));

  const readJsonlLines = (workDir: string): string[] => {
    const jsonlPath = evoResearchJsonlPath(workDir);
  ...
  const readLastRun = (workDir: string): Record<string, unknown> | null => {
    ...
    for (let i = lines.length - 1; i >= 0; i--) {
      ...
      if (isEvoResearchRunEntry(entry)) return entry;
    }
    return null;
  };
  ...
  const fireHook = async (payload: HookPayload): Promise<string | null> => {
    ...
    appendHookLogEntryIfConfigured(evoResearchJsonlPath(payload.cwd), payload.event, result);
  ...
  const clearSessionUi = (ctx: ExtensionContext) => {
    clearOverlay();
    if (ctx.hasUI) {
      ctx.ui.setWidget("evo-research", undefined);
    }
  };

  const evoResearchHelp = () =>
    [
      "Usage: /evo-research [off|clear|export|<text>]",
      "",
      "<text> enters evo-research mode and starts or resumes the loop.",
      "off leaves evo-research mode.",
      "clear deletes evo-research.jsonl and turns evo-research mode off.",
      "export opens a local live dashboard for evo-research.jsonl in your browser.",

      "",
      "Examples:",
      "  /evo-research optimize unit test runtime, monitor correctness",
      "  /evo-research model training, run 5 minutes of train.py and note the loss ratio as optimization target",
      "  /evo-research export",
    ].join("\n");
  ...
  const reconstructState = (ctx: ExtensionContext) => {
    ...
    // Primary: read from evo-research.jsonl (alongside evo-research.md/sh)
    const jsonlPath = evoResearchJsonlPath(workDir);
    ...
    // Auto-enter evo-research mode only when a persisted experiment log exists
    runtime.evoResearchMode = fs.existsSync(evoResearchJsonlPath(workDir));
  ...
  const updateWidget = (ctx: ExtensionContext) => {
    ...
    if (state.results.length === 0) {
      ...
      ctx.ui.setWidget("evo-research", (tui, theme) => ({
      ...
      ctx.ui.setWidget("evo-research", (tui, theme) => ({
        render(width: number): string[] {
          ...
          const title = truncateDisplayText(
            `🧬 evo-research${state.name ? `: ${state.name}` : ""}`,
  ...
  const ensurePendingResume = (
    ctx: ExtensionContext,
    gate: (runtime: EvoResearchRuntime) => boolean,
  ...
  pi.on("session_before_compact", async (event, ctx) => {
    ...
    return evoResearchCompactionFor(ctx, event);
  });
  ...
  // When in evo-research mode, add a static note to the system prompt.
  // Only a short pointer — no file content, fully cache-safe.
  pi.on("before_agent_start", async (event, ctx) => {
    const runtime = getRuntime(ctx);
    if (!runtime.evoResearchMode) return;

    const workDir = resolveWorkDir(ctx.cwd);
    const mdPath = evoResearchMdPath(workDir);
    ...
    let extra =
      "\n\n## EvoResearch Mode (ACTIVE)" +
      "\nYou are in evo-research mode. Optimize the primary metric through an autonomous experiment loop." +
      ...
      "\nWrite promising but deferred optimizations as bullet points to evo-research.ideas.md — don't let good ideas get lost." +
      `\n${BENCHMARK_GUARDRAIL}` +
  ...
  pi.registerTool({
    ...
    description:
      "Initialize the experiment session. Call once before the first run_experiment to set the name, primary metric, unit, and direction. Writes the config header to evo-research.jsonl.",
    promptSnippet:
      "Initialize experiment session (name, metric, unit, direction). Call once before first run.",
    promptGuidelines: [
      "Call init_experiment exactly once at the start of an evo-research session, before the first run_experiment.",
      "If evo-research.jsonl already exists with a config, do NOT call init_experiment again.",
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      try {
        ...
        return {
          content: [{
            type: "text",
            text: `⚠️ Failed to write evo-research.jsonl: ${e instanceof Error ? e.message : String(e)}`,
          }],
          details: {},
        };
      }

      const wasInactive = !runtime.evoResearchMode;
      runtime.evoResearchMode = true;
      updateWidget(ctx);
  ...
  pi.registerTool({
    ...
    description:
      `Run a shell command as an experiment. Times wall-clock duration, captures output, detects pass/fail via exit code. Output is truncated to last ${EXPERIMENT_MAX_LINES} lines or ${EXPERIMENT_MAX_BYTES / 1024}KB (whichever is hit first). If truncated, full output is saved to a temp file. Use for any evo-research experiment.`,
    promptSnippet:
    ...
    async execute(_toolCallId, params, signal, onUpdate, ctx) {
      ...
      if (evoResearchShExists && !isEvoResearchShCommand(params.command)) {
        return {
          content: [{
            type: "text",
            text: `❌ evo-research.sh exists — you must run it instead of a custom command.\n\nFound: ${evoResearchShPath}\nYour command: ${params.command}\n\nUse: run_experiment({ command: "bash evo-research.sh" }) or run_experiment({ command: "./evo-research.sh" })`,
          }],
          details: {
      ...
      if (details.timedOut) {
        ...
        text += `⏰ CHECKS TIMEOUT (evo-research.checks.sh) after ${checksDuration.toFixed(1)}s\n`;
        text += `Log this as 'checks_failed' — the benchmark metric is valid but checks timed out.\n`;
      } else if (checksPass === false) {
        text += `✅ Benchmark PASSED in ${durationSeconds.toFixed(1)}s\n`;
        text += `💥 CHECKS FAILED (evo-research.checks.sh) in ${checksDuration.toFixed(1)}s\n`;
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      ...
      "log_experiment automatically runs git add -A && git commit on 'keep', and auto-reverts code changes on 'discard'/'crash'/'checks_failed' (evo-research files are preserved). Do NOT commit or revert manually.",
      ...
      "If you discover complex but promising optimizations you won't pursue immediately, append them as bullet points to evo-research.ideas.md. Don't let good ideas get lost.",
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      try {
        fs.appendFileSync(evoResearchJsonlPath(workDir), jsonlLine + "\n");
        broadcastDashboardUpdate(workDir);
      } catch (e) {
        text += `\n⚠️ Failed to write evo-research.jsonl: ${e instanceof Error ? e.message : String(e)}`;
      }

      if (params.status !== "keep") {
        try {
          const revertScript = `
            git checkout -- . ':(exclude,glob)**/evo-research.*' ':(exclude,glob)**/evo-research.*/**'
            git clean -fd -e 'evo-research.*' -e '**/evo-research.*/**' 2>/dev/null
          `;
          await pi.exec("bash", ["-c", revertScript], { cwd: workDir, timeout: 10000 });
          text += `\n📝 Git: reverted changes (${params.status}) — evo-research files preserved`;
        } catch (e) {
  ...
  if (shortcuts.toggleDashboard) {
    pi.registerShortcut(shortcuts.toggleDashboard, {
      description: "Toggle evo-research dashboard",
      handler: async (ctx) => {
        const runtime = getRuntime(ctx);
        const state = runtime.state;
        if (state.results.length === 0) {
          if (!runtime.evoResearchMode && !fs.existsSync(evoResearchMdPath(resolveWorkDir(ctx.cwd)))) {
            ctx.ui.notify("No experiments yet — run /evo-research to get started", "info");
          } else {
  ...
  if (shortcuts.fullscreenDashboard) {
    pi.registerShortcut(shortcuts.fullscreenDashboard, {
      description: "Fullscreen evo-research dashboard",
      handler: async (ctx) => {
        const runtime = getRuntime(ctx);
        const state = runtime.state;
        if (state.results.length === 0) {
  ...
  function readJsonlContent(workDir: string): string {
    return fs.readFileSync(evoResearchJsonlPath(workDir), "utf-8").trim();
  }
  ...
  function resolveServedFile(workDir: string, requestPath: string): string | null {
    ...
    if (requestPath === "/evo-research.jsonl") return evoResearchJsonlPath(workDir);
    return null;
  }
  ...
  async function exportDashboard(ctx: ExtensionContext): Promise<void> {
    ...
    if (!fs.existsSync(jsonlPath)) {
      ctx.ui.notify("No evo-research.jsonl found \u2014 run some experiments first", "error");
      return;
    }
  ...
  // -----------------------------------------------------------------------
  // /evo-research command — enter evo-research mode
  // -----------------------------------------------------------------------

  pi.registerCommand("evo-research", {
    description: "Start, stop, clear, or resume evo-research mode",
    handler: async (args, ctx) => {
      ...
      if (!trimmedArgs) {
        ctx.ui.notify(evoResearchHelp(), "info");
        return;
      }
      ...
      if (command === "off") {
        ...
        ctx.ui.notify(
          wasRunning ? "EvoResearch mode OFF — aborting current run" : "EvoResearch mode OFF",
          "info"
        );
        return;
      }
      ...
      if (command === "clear") {
        const jsonlPath = evoResearchJsonlPath(resolveWorkDir(ctx.cwd));
        runtime.evoResearchMode = false;
        runtime.dashboardExpanded = false;
        runtime.autoResumeTurns = 0;
        runtime.experimentsThisSession = 0;
        ...
        if (fs.existsSync(jsonlPath)) {
          try {
            ...
            ctx.ui.notify("Deleted evo-research.jsonl and turned evo-research mode OFF", "info");
          } catch (error) {
            ctx.ui.notify(
              `Failed to delete evo-research.jsonl: ${error instanceof Error ? error.message : String(error)}`,
              "error"
            );
          }
        } else {
          ctx.ui.notify("No evo-research.jsonl found. EvoResearch mode OFF", "info");
        }
        return;
      }

      if (runtime.evoResearchMode) {
        ctx.ui.notify("EvoResearch already active — use '/evo-research off' to stop first", "info");
        return;
      }
      ...
      const workDir = resolveWorkDir(ctx.cwd);
      const rulesLoaded = hasEvoResearchRules(ctx);
      const kickoff = rulesLoaded
        ? `EvoResearch mode active. ${trimmedArgs} ${BENCHMARK_GUARDRAIL}`
        : `Start evo-research: ${trimmedArgs} ${BENCHMARK_GUARDRAIL}`;

      ctx.ui.notify(
        rulesLoaded
          ? "EvoResearch mode ON — rules loaded from evo-research.md"
          : "EvoResearch mode ON — no evo-research.md found, setting up",
        "info",
      );
