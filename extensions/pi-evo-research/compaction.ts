/**
 * Deterministic compaction summary for evo-research sessions.
 *
...
export interface EvoResearchSummaryPaths {
  workDir: string;
  jsonlPath: string;
  mdPath: string;
  ideasPath: string;
...
export function evoResearchSummaryPathsFor(workDir: string): EvoResearchSummaryPaths {
  return {
    workDir,
    jsonlPath: path.join(workDir, "evo-research.jsonl"),
    mdPath: path.join(workDir, "evo-research.md"),
    ideasPath: path.join(workDir, "evo-research.ideas.md"),
  };
}

/**
 * Build the full compaction summary text from persisted evo-research state.
 * Returns a markdown string that is itself the entire compaction summary.
 */
export function buildEvoResearchCompactionSummary(paths: EvoResearchSummaryPaths): string {
...
function headerSection(): string {
  return [
    "# EvoResearch Compaction Summary",
    "",
    "The conversation history was discarded; the persisted evo-research state below is the source of truth.",
...
function recentRunsSection(state: ReconstructedJsonlState): string {
  ...
  return [
    ...
    ...lines,
    "",
    "If you need more details, read additional lines from evo-research.jsonl.",
  ].join("\n");
}
