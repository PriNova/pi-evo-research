#!/usr/bin/env bash
# When evo-research.md exceeds a size threshold, keep the preamble and
...
input="$(cat)"
md="$(jq -r '.cwd' <<<"$input")/evo-research.md"
[ -f "$md" ] && file_too_large "$md" || exit 0

archive_tail "$md"
echo "Rotated evo-research.md (kept first $KEEP_LINES lines, archived rest)."
