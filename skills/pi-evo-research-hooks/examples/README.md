...
```bash
cp "<skill-dir>/examples/before/external-search.sh" /path/to/session/evo-research.hooks/before.sh
...
| [`idea-rotator.sh`](before/idea-rotator.sh) | Surface the next unchecked bullet from `evo-research.ideas.md` as a steer nudge. |
...
| [`context-rotation.sh`](before/context-rotation.sh) | Trim an oversized `evo-research.md`, archiving the tail. |
