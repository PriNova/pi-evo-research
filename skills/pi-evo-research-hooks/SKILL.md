---
name: pi-evo-research-hooks
description: Author pre/post-iteration hooks for an evo-research session. Use when the user asks to add research fetching, Slack/webhook notifications, persistent learnings, auto-tagging, anti-thrash intervention, idea rotation, or any side effect around iterations.
---
...
`evo-research.hooks/**` survives the auto-revert, like all paths matching `evo-research.*`.
...
1. **Understand the session.** Read `evo-research.md` for the objective and metric; glance at `evo-research.sh` for the workload. Your hook should complement the loop, not duplicate it.
...
6. **Commit the hook** alongside other session files. It's preserved across reverts because the path matches `evo-research.*`.
