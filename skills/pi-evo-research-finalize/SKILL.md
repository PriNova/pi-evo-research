---
name: pi-evo-research-finalize
context: fork
description: Finalize an evo-research session into clean, reviewable branches. Use when asked to "finalize evo-research", "clean up experiments", or "prepare evo-research for review".
---

# Finalize Evo Research

Turn a noisy evo-research branch into clean, independent branches — one per logical change, each starting from the merge-base.

## Step 1 — Analyze and Propose Groups

1. Read `evo-research.jsonl`. Filter to **kept** experiments only.
2. Read `evo-research.md` for context.
...
The script creates one branch per group from the merge-base, verifies the union matches the original branch, and prints a summary with all branches, cleanup commands, and any ideas from `evo-research.ideas.md`.
