...
# pi-evo-research-finalize — creates independent branches from an evo-research session
#
...
#   "trunk": "main",
#   "final_tree": "<full HEAD hash of evo-research branch>",
#   "goal": "short-slug",
#   "groups": [
#     {
...
assert_on_feature_branch() {
  ...
  [ -n "$ORIG_BRANCH" ] || fail "Detached HEAD — switch to the evo-research branch first."
  [ "$ORIG_BRANCH" != "$TRUNK" ] || fail "On trunk ($TRUNK) — switch to the evo-research branch first."
}
...
preflight() {
  ...
  for i in $(seq 0 $((GROUP_COUNT - 1))); do
    ...
    branch_name="evo-research/${GOAL}/${group_number}-$(cat "$DATA_DIR/$i.slug")"
  ...
  assert_branch_available "evo-research/${GOAL}/verify-tmp"
...
create_group_branch() {
  ...
  group_number=$(printf "%02d" $((i + 1)))
  branch_name="evo-research/${GOAL}/${group_number}-${slug}"
...
verify_union_matches_original() {
  local verify_branch="evo-research/${GOAL}/verify-tmp"
  ...
  if [ -n "$non_session_diff" ]; then
    echo -e "${RED}✗ Union of groups differs from evo-research branch!${NC}"
    echo "  Files:$non_session_diff"
    return 1
  fi

  echo -e "${GREEN}✓ Union of all groups matches original evo-research branch.${NC}"
  return 0
}
...
print_summary() {
  ...
  echo ""
  echo "Cleanup — after merging, delete the evo-research branch and session files:"
  echo ""
  echo "  git branch -D $ORIG_BRANCH"
  echo "  rm -f evo-research.jsonl evo-research.sh evo-research.md evo-research.ideas.md"

  if [ -f "evo-research.ideas.md" ]; then
    echo ""
    echo "Ideas backlog (from evo-research.ideas.md):"
    echo ""
    sed 's/^/  /' evo-research.ideas.md
  fi
